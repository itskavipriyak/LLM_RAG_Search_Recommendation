...
def load_all_documents(data_dir: str) -> List[Any]:
    ...
    print(f"[DEBUG] Found {len(jpg_files)} JPG files: {[str(f) for f in jpg_files]}")
    for jpg_files in jpg_files:
        print(f"[DEBUG] Loading JPG: {jpg_files}")
        try:
            loader = UnstructuredImageLoader(str(jpg_files))
            loaded = loader.load()
            print(f"[DEBUG] Loaded {len(loaded)} JPG docs from {jpg_files}")
            documents.extend(loaded)
        except Exception as e:
            print(f"[ERROR] Failed to load JPG {jpg_files}: {e}")
